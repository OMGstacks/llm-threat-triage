#!/usr/bin/env python3
"""cli.py — project entrypoint wiring for the repo root (llm-log-triage).
...
#
# CAUTION: PROJECT_ROOT here is the *repo root*, not a project subdirectory
# -- EnvVarIndexGenerator would scan `**/*.py` across projects/, red-team/,
# and every other directory. It is deliberately never gated/committed (see
# above), so this breadth is harmless, but don't `--write --only
# env_var_index` and commit the result without checking its scope first.
