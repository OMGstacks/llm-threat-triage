...
repo has already hit the failure mode it's meant to prevent: two toolkit
install commits landed on `claude/cc-cert-learning-plan-xqo5g7` mid-session
from a different actor while another session was actively committing PR work
on the same branch (see `MEMORY.md`'s Current State). Nothing broke that
time only because it was caught by hand on every rebase.
...
```json
{
  ...
  "branch": "claude/cognition-substrate",
  "files_scope": ["cc-master-learning-center/.cognition/active-work/"],
