...
1. Inspect the primary checkout's current state (branch, divergence) without touching it,
   run from inside the repo checkout (never hardcode its path — that varies per
   machine/session; see CORE.md's worktree-isolation convention below). `git worktree`
   operates on the whole repo even when run from this project's subdirectory:
   `git worktree list`
...
3. Create an ISOLATED workspace (a worktree, a separate clone, or your project's
   ...
   directory to the repo checkout (this creates the whole repo there; work inside its
   `cc-master-learning-center/` subdirectory):
...
1. **Scope.** From inside the isolated workspace's `cc-master-learning-center/` subdirectory
   (`../llm-threat-triage-wt-<task>/cc-master-learning-center`): `git status --porcelain` and
   `git diff --name-only` must contain ONLY files this task owns. A foreign path or stray
   file FAILS the gate — go back and unstage/revert it.
