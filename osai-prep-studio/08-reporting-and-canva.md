...
> **Implementation status.** The **offline fallback path is live** as `osai_spine.studypack` (pure, stdlib-only functions over the learner's [`/analytics`](05-progress-engine.md) payload) exposed at `GET /export/{learner}/{artifact}` for four artifacts: **`flashcards.csv`** (Anki/Quizlet `front,back,tags` deck seeded from weak topics + unfinished labs), **`studypack.md`** (readiness + missed-framework heatmap + weakest-first drill list + lab→topic plan), **`deck.md`** (a **Marp** slide deck), and **`labmap.mmd`** (a **Mermaid** flowchart of the lab→topic map, labs colored by pass status). The web `/analytics` page downloads them client-side (works in Bearer and cookie auth). Every artifact is **answer-key-safe** — built from public framework tags and the *concept* of the evidence token, never a flag value or the redacted `detector_required` — and unit-tested (`test_studypack.py`, plus an endpoint test). The **Canva Connect path** layers on the *same* generated content (import the Marp `deck.md` / the CSV) and remains an optional, OAuth-gated enhancement, not a runtime dependency.
