...
| `tests/` | 84 pytest tests: taxonomy, flags, manifests, grading (incl. cross-framing labs), the **attack→target→grade loops** (L01/L02/L11) + **L10/L13/L14 grading**, the **tutor** + **LLM seam** + **redaction** + **gold-set ship gate** + **scope guard**, the **progress engine** + **badges/leaderboard** + **SM-2 flashcards**, the **Report-Reviewer**, the **Exam Simulator**, the **web UI**, the **stdlib HTTP service**, and the **FastAPI app** | — |
