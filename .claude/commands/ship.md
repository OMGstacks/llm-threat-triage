...
**Automated registry check (this toolkit install is scoped to `projects/llm-log-triage`;
run after the fresh fetch, before touching anything).** The manual grep above only catches
what's already visible in PR/branch names. Also check and register against the live claims
registry (`.cognition/active-work/`, see its `README.md`):
