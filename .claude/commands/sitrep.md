...
- Cross-session/teammate work-in-progress map, if the project has one:
  `not applicable — no such tool exists for this project; use the documented fallback above` — otherwise fall back to the concurrency-check steps in
...
| Cross-session coordination | `not applicable — no such tool exists for this project; use the documented fallback above` (fallback: concurrency-check.md) |
