...
**Release any concurrency claim first (osai-prep-studio).** If this session registered a
claim via `claims_cli.py claim` (see Phase 0 of `ship.md`), release it now so other sessions
stop seeing this scope as active the moment they next `git fetch`:
