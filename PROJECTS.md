...
- **CI workflow:** `.github/workflows/ci.yml` — runs on every push/PR to `main`/`master`.
- **Test entrypoint:** `python -m pytest -q` from `projects/llm-log-triage/`.
- **Memory file:** none yet — out of scope for the current cognition-substrate slice, which
  is proving the pattern on `cc-master-learning-center` first before porting here.
...
- **Governance doc:** a chronological series of 25+ numbered planning docs at its root
  ...
  distinct from the `CLAUDE.md`/memory pattern used elsewhere in this repo, and one worth
  respecting rather than replacing when the cognition-substrate port lands in a later slice.
  Also carries a Cognition OS toolkit install (`.cognition/`, `.claude/`), currently inert (no
  real `MEMORY.md`/`gates.yml`/friction log instantiated).
- **CI workflow:** `.github/workflows/osai-spine.yml` (path-filtered to
  ...
  `reference/**`, and one shared file in `projects/llm-log-triage`).
...
- **Memory file:** none yet — deferred to the Slice 5 port (see cognition-substrate plan).
