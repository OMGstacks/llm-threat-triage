...
> portfolio project (`projects/llm-log-triage`, `red-team/`, `reference/`) has no remote
> deploy target — it's demonstrated as "zero setup (Python standard library only)" per
> the root README, run entirely via local CLI. There is nothing to SSH into, no host to
> administer, no runtime identity to check. Do not invoke this command for this project;
> the placeholders below were never filled in because there is no deploy surface to fill
> them with. If this project ever gains one, fill in every `<LIKE_THIS>` token first.
