...
> **NOT APPLICABLE to this project (confirmed 2026-07-07).** OSAI Prep Studio has no
> `migrations/` directory and no versioned schema — its fact store
> (`spine/facts/`, `spine/banks/`, `spine/gold/`) is flat JSON, versioned by git
> history directly. There is no live production database to gate an apply against
> (see `PROJECT_FACTS.yml`). Do not invoke this command for this project; the
> placeholders below were never filled in because there is no migration surface to
> fill them with. If this project ever adopts versioned migrations, fill in every
> `<LIKE_THIS>` token first.
